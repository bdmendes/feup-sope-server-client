...
void* producer(void* arg){
    ...
    message.tskres = answer; // it is up to the consumer to change the thread and process ids
                             // after popping from the ready queue, after gathering the private fifo info to send
    pthread_mutex_lock(&ready_mutex);
    message_queue_push(ready, &message); // should we also limit the ready queue size?
